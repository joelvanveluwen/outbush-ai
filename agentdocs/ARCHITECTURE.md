...
- `/api/health` reports whether SQLite, llama.cpp/Nemotron, MiniCPM-V, and the species classifier are active.
...
The model paths are additive. The deterministic fallback is the safety floor; llama.cpp/Nemotron, MiniCPM-V, and the field-tuned classifier improve specificity but do not replace guardrails.
